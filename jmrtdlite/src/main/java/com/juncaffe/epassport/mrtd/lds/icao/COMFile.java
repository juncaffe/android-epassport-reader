...
import com.juncaffe.epassport.mrtd.lds.AbstractTaggedLDSFile;
...
public class COMFile extends AbstractTaggedLDSFile {
  ...
  public COMFile(InputStream in) throws IOException {
    super(EF_COM_TAG, in, null);
  }

  @Override
  protected void readContent(InputStream in) throws IOException {
    TLVInputStream tlvIn = in instanceof TLVInputStream ? (TLVInputStream)in : new TLVInputStream(in, null);
  ...
  public String toString() {
    ...
    for (int tag: tagList) {
      result.append("DG" + Utils.lookupDataGroupNumberByTag(tag));
      if (i < dgCount - 1) {
        result.append(", ");
      }
      i++;
