...
public class ReadBinaryAPDUSender {
   ...
   */
  public synchronized byte[] sendReadBinary(APDUWrapper wrapper, int offset, int le, boolean isTLVEncodedOffsetNeeded) throws CardServiceException {
  ...
  private static void checkStatusWordAfterFileOperation(CommandAPDU commandAPDU, ResponseAPDU responseAPDU) throws CardServiceException {
    ...
    if ((sw & ISO7816.SW_WRONG_LENGTH) == ISO7816.SW_WRONG_LENGTH && data.length == 0) {
